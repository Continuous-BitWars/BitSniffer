...
import {BoardAction} from "../../models/game/boardActions";
import {Color, Group, Object3D, Vector3} from "three";
import {ModelService, PI} from "../modelService/model.service";
import {BaseService} from "../baseService/base.service";
import {CacheAttack} from "../../models/cache/cache-attack";
...
export class AttackService {
  ...
  constructor(private modelService: ModelService, private baseService: BaseService, private playerService: PlayerService) {};
    ...
    if (attackIndex === -1) {
      ...
      this.attacks[attackIndex].model.traverse((model): void => {
        ...
        if (model.name.includes("teamColor")) {
          ...
          // set color from HEX
          (<any>model).material.color = new Color(this.playerService.getPlayer(attack.player).color);
  ...
  private addFont(attackIndex: number, amount: number): void {
    ...
    // rotate both fonts
    fontFront.rotation.set(0, PI/2,  0);
    fontBack.rotation.set(0, 3*PI/2,  0)
